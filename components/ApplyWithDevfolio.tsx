...
export default function ApplyWithDevfolio({ disabled, smaller }: { disabled?: boolean; smaller: boolean; }) {
  ...
  return (
    <Button disabled={disabled} onClick={() => open(DEVFOLIO_LINK)} className={cn("flex bg-main h-auto p-0", smaller ? "w-[40%]" : "w-[80%]")}>
      <div className="flex items-stretch w-full h-full justify-between font-black">
        <div className={cn("self-center text-4xl grow-1 text-center text-white", hideDevfolioIcon ? "p-8" : "pl-8 pr-8")}>Apply with Devfolio</div>
        { hideDevfolioIcon || (
