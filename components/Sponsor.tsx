...
import { NORMAL_SPONSOR, SOCIALS, SPONSOR_PACKET, SPONSORS } from "@/app/config"
      ...
      <div className={cn("flex flex-col w-[100vw] gap-15 justify-evenly items-center text-center", verySmall ? "p-2" : "p-8")}>
        <div className="flex flex-col gap-4">
          {Object.entries(SPONSORS).map(([title, sponsors], i) => (
            <Card className={cn("flex flex-col gap-4 p-4 justify-center", aBitSmall ? "w-[90vw]" : "w-[50vw]")}>
              ...
              <div className={cn("grid gap-4", twoColumns ? "grid-cols-1" : "grid-cols-3")}>
                {sponsors.map(({ image_link, link, name }, j) => (
                  <Card key={`sponsor-${name}`} className={cn("flex items-center justify-center p-8", SHADOWCOLOR[(i + j) % SHADOWCOLOR.length] + "shadow")}>
                    <Link href={link}>
